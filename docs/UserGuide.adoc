...
You can activate the Pomodoro timer with a task in focus. You can add an optional timer amount field, different from the default time of 25 minutes, for that particular cycle.
...
===== Break-time
After, the app will prompt you if you would like to begin a 5 minute break (as per the pomodoro technique). Similarly, you need to respond with Yes (Y) or No (N). Responding with No will set the app to its neutral state. Yes will cause the break timer to run.

===== Back to work
At the end of the break, the app will once again prompt you if you would like to continue. You can respond with No (N) or call the `pom` command on another undone task to continue. 

...
Pomodoro's features are implemented mainly in `seedu.address.logic` package. The `PomodoroManager` class is used to manipulate the timer and configure the relevant UI elements.. The timer is facilitated by `javafx.animation.Timeline`. 

Through the use of the Pomodoro feature, there are occassions where the app has to prompt the user for specific input in order to progress. This behaviour flow is represented in the _Pomodoro Flow_ diagram.

_{Insert Pomodoro Prompt Flow diagram here}_
...
* `CHECK_TAKE_BREAK`: This state occurs _after_ user response has been received in the `CHECK_DONE` state.
...
This data is captured and stored in the `Pomodoro` class in `seedu.address.model`, which interacts with the app's storage system. `PomodoroManager` also updates `Pomodoro` model on what task is being run and the time remaining in a particular cycle. This allows the time progress to be persistent in between app closures and relaunches.

=== Statistics

The user's Pomodoro usage is tracked daily by tabulating the total duration when the Pomodoro is running, denoted by PomDurationData, and the number of tasks done, denoted by TasksDoneData. Data from the Pomodoro is stored in a DayData object that consist of the fields Date, PomDurationData and TasksDoneData. 

A CustomQueue is used to store the dataset of DayDatas for the past 7 days. On creation, CustomQueue initialises with 7 DayDatas with the dates for the past 7 days and a value of 0 for both PomDurationData and TasksDoneData. DayData is updated through method calls by the Pomodoro in ModelManager. In the event that the user updates this dataset in the next/several days, CustomQueue will reinitialise the dates, maintaining only data for dates which still lie within 7 days from the new day. Older data is popped and discarded.

A bar graph is generated based on this dataset and displayed on the Statistics panel for the user to track Pomodoro usage for the past 7 days.
...
1.  User requests to create a Task specifying a timing to create a Reminder.
2.  BBProductive creates the task and reflects it onto the task list.
3.  Reminder pops up when time is reached.
+
Use case ends.

*Extensions*

* 2a. A duplicate task is created.
+
[none]

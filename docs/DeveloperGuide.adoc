...
Step 4. `AddCommand` executes. It checks if the `TaskList` contains a duplicate `Task` to it through `Model#hasTask`. If a duplicate `Task` exists, a `DuplicateTaskException` is thrown. If not, it adds the new `Task` into the `Tasklist` through `Model#addTask`.
